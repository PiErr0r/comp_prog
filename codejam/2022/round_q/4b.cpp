...
void undone_graph(Node* n) {
  n->done = false;
  for (int i = 0; i < (int)n->children.size(); ++i) {
    undone_graph(n->children[i]);
  }
}

ll sum_nodes(Node* root, int i) {
  Node* start = find_node(root, i);
  ll res = 0;
  while (start->num != 0 && !start->done) {
    //return;
    res = max(res, start->fun);
    start->done = true;
    start = start->parent;
  }
...
void solve(void) {
  ...
  do {
    curr = 0;
    for (int j = 0; j < to_perm.size(); ++j) {
      curr += sum_nodes(root, to_perm[j]);
    }
    res = max(res, curr);
//    cout << "RES: " << res << " CURR: " << curr << endl;
    undone_graph(root);
